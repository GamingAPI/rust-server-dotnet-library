...
namespace Asyncapi.Nats.Client.Channels
{
  ...
  internal static ServerPlayerItemCrafted JsonDeserializerSupport(LoggingInterface logger, byte[] buffer)
{
  var srt = Encoding.UTF8.GetString(buffer);
  logger.Debug("Deserializing message " + srt);
  return JsonSerializer.Deserialize<ServerPlayerItemCrafted>(srt);
}
...
String server_id,String steam_id,String item_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.crafted");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.crafted";
...
itemIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.crafted");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.crafted",handler);
  }
