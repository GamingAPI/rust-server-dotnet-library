...
namespace Asyncapi.Nats.Client.Channels
{
  ...
  internal static ServerPlayerReported JsonDeserializerSupport(LoggingInterface logger, byte[] buffer)
{
  var srt = Encoding.UTF8.GetString(buffer);
  logger.Debug("Deserializing message " + srt);
  return JsonSerializer.Deserialize<ServerPlayerReported>(srt);
}
...
String server_id,String steam_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.reported");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.reported";
...
steamIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.reported");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.reported",handler);
  }
