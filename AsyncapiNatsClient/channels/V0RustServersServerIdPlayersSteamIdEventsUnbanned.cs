...
namespace Asyncapi.Nats.Client.Channels
{
  ...
  internal static ServerPlayerUnbanned JsonDeserializerSupport(LoggingInterface logger, byte[] buffer)
{
  var srt = Encoding.UTF8.GetString(buffer);
  logger.Debug("Deserializing message " + srt);
  return JsonSerializer.Deserialize<ServerPlayerUnbanned>(srt);
}
...
String server_id,String steam_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.unbanned");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.unbanned";
...
steamIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.unbanned");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.unbanned",handler);
  }
