...
namespace Asyncapi.Nats.Client.Channels
{
  ...
  internal static ServerPlayerDisconnected JsonDeserializerSupport(LoggingInterface logger, byte[] buffer)
{
  var srt = Encoding.UTF8.GetString(buffer);
  logger.Debug("Deserializing message " + srt);
  return JsonSerializer.Deserialize<ServerPlayerDisconnected>(srt);
}
...
String server_id,String steam_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.disconnected");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.disconnected";
...
steamIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.disconnected");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.disconnected",handler);
  }
