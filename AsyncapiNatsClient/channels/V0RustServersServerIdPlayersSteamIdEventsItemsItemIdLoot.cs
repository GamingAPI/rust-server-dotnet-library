...
namespace Asyncapi.Nats.Client.Channels
{
  ...
  internal static ServerPlayerItemLoot JsonDeserializerSupport(LoggingInterface logger, byte[] buffer)
{
  var srt = Encoding.UTF8.GetString(buffer);
  logger.Debug("Deserializing message " + srt);
  return JsonSerializer.Deserialize<ServerPlayerItemLoot>(srt);
}
...
String server_id,String steam_id,String item_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot";
...
itemIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot",handler);
  }
